<template>
  <div class="mod-config">
    <el-form :inline="true">
      <el-form-item>
        <el-button type="primary" @click="addWeek(addPrice)">新增排班</el-button>
...
<script>
import AddOrUpdate from "./registeredScheduling-add-or-update";
export default {
  data() {
    return {
      dataList: [],
  ...
  methods: {
    ...
    addWeek(addPrice) {
      ...
      this.$nextTick(() => {
      this.getDataList();
        this.$refs.addOrUpdate.init(addPrice);
      });
    },
    //停止循环
    stopCycle(row) {
