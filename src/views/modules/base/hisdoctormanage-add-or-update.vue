    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sys/hisdoctormanage/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.hisdoctormanage.name
                this.dataForm.describe = data.hisdoctormanage.describe
                this.dataForm.address = data.hisdoctormanage.address
                this.dataForm.createDate = data.hisdoctormanage.createDate
                this.dataForm.status = data.hisdoctormanage.status
                this.dataForm.check = data.hisdoctormanage.check
              }
            })
          }
        })
