    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button type="success" size="small" @click="addOrUpdateHandle(scope.row.id)">编辑</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/recipeTemplate/findRecipeTemplateList'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            'templateName': this.dataForm.paramKey
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/recipeTemplate/delRecipeTemp'),
            method: 'post',
