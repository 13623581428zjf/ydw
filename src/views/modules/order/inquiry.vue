    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
           <span>{{scope.row.scheduling_type == 'video'?'视频':'图文'}}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="remark"
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="seeDetails(scope.row)">详情</el-button>
          <el-button type="text" size="small" @click="" v-if='scope.row.status=="complete"'>查看病例</el-button>
          <el-button type="text" size="small" @click="startCslt(scope.row)" v-if='scope.row.status=="paid" && (scope.row.consultation_status =="Y" || scope.row.consultation_status == "")' >发起问诊</el-button>
          <el-button type="text" size="small" @click="" v-if='scope.row.status=="paid" && scope.row.consultation_status !="Y" ' >编辑病例</el-button>
          <el-button type="text" size="small" v-if='new Date(new Date(`${scope.row.consultation_time} 00:00:00`).getTime()-3600*1000*12) > new Date() && scope.row.consultation_status !="N" && (scope.row.status == "paid" || scope.row.status == "created")'  @click="offBespeak(scope.row.id)">取消预约</el-button>
          <el-button type="text" size="small" @click="endOrder(scope.row)"  v-if='scope.row.status=="paid" && scope.row.consultation_status=="Y" '>结束问诊</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
import OrderDetails from "./order-iInquisitionin-details";
export default {
  data() {
    return {
      dataForm: {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.dataListLoading = true;
        this.$http
          .getRequest("/inquisition/findInquisition", {
